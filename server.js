...
app.get('/', (req, res) => {
    const {name, height, weight, startdate, enddate, married, mode} = req.query
    const queries = []
    const params = []
    ...
    if(startdate && enddate) {
        queries.push(`between ? and ?`)
        params.push(startdate, enddate)
    } else if(startdate) {
        queries.push('birthdate >= startdate')
        params.push(startdate)
    } else if (enddate) {
        queries.push('birthdate <= endtdate')
        params.push(enddate)
    }
